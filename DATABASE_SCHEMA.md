...
The platform uses a lightweight mapping table `user_auth_accounts` to associate one or more auth identities (`auth_id`) with a single canonical `users` profile (`user_id`). This avoids destructive primary-key reassignments when a user signs in with an auth identity that shares an email with an existing profile.

- Table: `user_auth_accounts(auth_id UUID PRIMARY KEY, user_id UUID REFERENCES users(id), created_at TIMESTAMPTZ)`
- Purpose: Allow multiple auth identities to map to one profile safely, and make it possible to "merge" or associate accounts without changing existing PKs or FK references.

## Inspection slots and scheduling

A new scheduling table `inspection_slots` is used to model administratively-created inspection time slots (optionally scoped to a specific car). It is used together with the `inspections` table to prevent double-booking and to show available/ booked slots in the UI.

- Table: `inspection_slots(id UUID PRIMARY KEY, car_id UUID NULL, start_at TIMESTAMPTZ NOT NULL, end_at TIMESTAMPTZ NOT NULL, capacity INTEGER DEFAULT 1, created_by UUID NULL, created_at TIMESTAMPTZ DEFAULT now())`
- Changes: `inspections` now includes `slot_id UUID NULL` (FK to `inspection_slots`) and `scheduled_time TIMESTAMPTZ NULL` (normalized scheduled timestamp).
- Purpose: Provide capacity-controlled slots admins can create and users can reserve. Server validation prevents bookings beyond capacity and avoids double-booking for the same car/time.
