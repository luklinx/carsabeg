...
"use client";
import { Car, ComponentCarCarouselProps } from "@/types";
...
export default function CarCarousel({ cars }: ComponentCarCarouselProps) {
  ...
  return (
    <div className="relative group w-full">
      ...
      {validCars.length > 1 && (
        <>
          <CustomButton onClick={prevSlide} containerStyles="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 bg-white/20 backdrop-blur-md hover:bg-white/40 text-white p-2 sm:p-3 md:p-4 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-lg" aria-label="Previous slide">
            <ChevronLeft size={20} className="sm:w-6 sm:h-6 md:w-8 md:h-8" />
          </CustomButton>
          <CustomButton onClick={nextSlide} containerStyles="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 bg-white/20 backdrop-blur-md hover:bg-white/40 text-white p-2 sm:p-3 md:p-4 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-lg" aria-label="Next slide">
        ...
        <div className="flex justify-center gap-2 mt-6 sm:mt-8">
            {validCars.map((_, i) => (
              <CustomButton
                key={i}
                onClick={() => setIndex(i)}
                containerStyles={`transition-all duration-500 rounded-full ${
                  i === index
