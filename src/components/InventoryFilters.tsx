...
export default function InventoryFilters({ initialCars }: Props) {
  ...
  const filteredCars = useMemo(() => {
    return initialCars.filter((car) => {
      ...
      if (
        currentState &&
        !(car.state || "").toLowerCase().includes(currentState.toLowerCase())
      )
        return false;
      if (
        currentCity &&
        !(car.city || "").toLowerCase().includes(currentCity.toLowerCase())
      )
        return false;
      return true;
    });
        ...
        <div className="container mx-auto px-6">
          {filteredCars.length > 0 ? (
            ...
            <div className="text-center py-32 bg-gray-50 rounded-3xl">
              <div className="text-4xl font-black text-gray-700 mb-8">
                No cars found
              </div>
