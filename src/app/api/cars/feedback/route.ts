...
export async function POST(req: NextRequest) {
  try {
    ...
    // Prefer authenticated user's email when available (safer for RLS policies)
    const {
      data: { user: authUser },
    } = await supabase.auth.getUser();

    const emailToUse = authUser?.email ?? user_email ?? null;

    if (!emailToUse) {
      return NextResponse.json(
        { error: "User email required (sign-in to leave feedback)" },
        { status: 401 }
      );
    }
